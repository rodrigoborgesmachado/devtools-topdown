...
namespace Model
{
    ...
    public class MD_Campos
    {
        ...
        /// <param name="projeto">Código do projeto</param>
        /// <returns>Lista das tabelas</returns>
        public static List<MD_Campos> RetornaCamposTabela(string campo, int tabela, int projeto)
        {
            string sentenca = new DAO.MD_Campos().table.CreateCommandSQLTable() + " WHERE CODIGOTABELA = " + tabela + " ORDER BY NOME";

            DbDataReader reader = DataBase.Connection.Select(sentenca);
            List<MD_Campos> tabelas = new List<MD_Campos>();

            while (reader.Read())
            {
                if (reader["NOME"].ToString().ToUpper().Contains(campo.ToUpper())
                    || reader["CODIGO"].ToString().Equals(campo))
                {
                    tabelas.Add(new MD_Campos(int.Parse(reader["CODIGO"].ToString()), tabela, projeto));
                }
            }
            reader.Close();
